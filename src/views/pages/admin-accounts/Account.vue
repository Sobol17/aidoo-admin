				...
				<Card class="h-full">
					...
					<template #content>
						<div class="flex flex-column gap-x-8 py-3">
							<div>
								<span class="font-semibold text-lg block mb-2">ФИО профилей сотрудника:</span>
								<p v-for="name in profilesNames" :key="name" class="m-0">{{ name }}</p>
							</div>
							<div>
							...
							<div class="h-full">
								...
								<div class="p-fluid p-3">
									<div class="field mb-5">
										<label for="phone" class="font-medium text-lg mb-3 block">Телефон</label>
										<InputText id="phone" v-model="formData.phone" :disabled="!editMode" />
									</div>

									<Divider />

									<div class="field mb-5">
										<label for="password" class="font-medium text-lg mb-3 block">Пароль</label>
										<Password
											id="password"
...
import { useAdminProfiles } from '@/composables/useAdminProfiles'
import { useProfileStore } from '@/stores/profile'
import Logs from '@/views/pages/logs/Logs.vue'
...
const { data: profilesData } = useAdminProfiles(
	route.params.id,
	profileStatus,
	profileType,
	'',
...
const profilesNames = computed(() => {
	return profilesData.value?.items?.map(profile => `${profile.firstName} ${profile.lastName}`) ?? []
})
