...
const parentCategory = categoriesData.find(category => category.code === route.params.id)

const { data: subcategoriesData, isLoading, error } = useSubcategories(route.params.id)
...
const { mutate: createSubcategory, isPending: isCreatingNewSubcategory } = useCreateSubcategory({
	onSuccess: () => {
		toast.add({
			severity: 'success',
			summary: 'Успех',
...
const { mutate: updateSubcategory, isPending: isUpdatingSubcategory } = useUpdateSubcategory({
	onSuccess: () => {
		toast.add({
			severity: 'success',
			summary: 'Успех',
...
function editSubcategory(subcategoryElement) {
	...
	const parentOption = categoriesData.find(option => option.code === subcategoryElement.category)
...
const { mutate: useDelete, isPending: isDeletingAccount } = useDeleteSubcategory({
	onSuccess: () => {
		toast.add({
			severity: 'success',
			summary: 'Успех',
	...
	<div>
		<div class="card">
			...
			>
				...
				<Column field="categoryName" header="Название родителя" style="min-width: 12rem"></Column>
				<Column field="category" header="Родитель (ID)" sortable style="min-width: 10rem"></Column>
				<Column field="id" header="ID" sortable style="min-width: 16rem"></Column>
				<Column field="isPremium" header="Премиум" sortable style="min-width: 12rem"></Column>
