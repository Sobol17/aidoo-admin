...
import { useSendMessage, useSupport, useSupportMessages } from '@/composables/useSupport'
	...
	<div>
		<div class="card">
			...
			>
				...
				></Column>
				<Column field="lastMessage" header="Последнее сообщение" sortable style="min-width: 12rem">
						...
						<div v-if="slotProps.data.user" class="p-4">
							<div class="flex gap-20">
								<div>
									...
									<div>
										Сообщение:
										<span class="text-lg">{{ slotProps.data.lastMessage.text }}</span>
									</div>
								</div>
								<div>
									<h5>Вложения</h5>
							...
							<DataTable :value="[slotProps.data.user]" removableSort>
		...
		>
			...
			<template #footer>
				<Button label="Закрыть" @click="messagesDialog = false" :loading="isSendingMessage" />
			</template>
		</Dialog>
	</div>
</template>
