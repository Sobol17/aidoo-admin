...
import { useCreatePromo, useDeletePromo, usePromo, useUpdatePromo } from '@/composables/usePromo'
...
const { data: promoData, isLoading: isPromosLoading } = usePromo(selectedPromoStatus)
...
const { mutate: useDeleteProfile, isPending: isDeletingProfile } = useDeletePromo({
	onSuccess: () => {
		toast.add({
			severity: 'success',
			summary: 'Успех',
	...
	<div>
		<div class="card">
			...
			>
				...
				<Column field="code" header="Промокод" sortable style="min-width: 8rem"></Column>
				<Column field="promoValue" header="Значение" sortable style="min-width: 6rem"></Column>
				<Column
					field="maxUses"
				...
				></Column>
				<Column field="promoStatus" header="Статус" sortable style="min-width: 10rem"></Column>
				<Column field="creator" header="Создатель (ID)" sortable style="min-width: 12rem"></Column>
			...
			<div class="flex flex-col gap-6">
				<div v-if="isEdit">
					...
					<InputText
						...
						disabled
					/>
					<small v-if="submitted && !newPromo.id" class="text-red-500">Обязательное поле</small>
				</div>
				<div v-if="isEdit">
					...
					/>
					<small v-if="submitted && !newPromo.status" class="text-red-500">Обязательное поле</small>
				</div>
				<div v-if="!isEdit">
					...
					<InputText
						...
						fluid
					/>
					<small v-if="submitted && !newPromo.code" class="text-red-500">Обязательное поле</small>
				</div>
				<div v-if="!isEdit">
				...
				<div v-if="!isEdit">
					<label for="name" class="block font-bold mb-3">Количество использований</label>
					<InputNumber
				...
				<div v-if="!isEdit">
					<label for="name" class="block font-bold mb-3">Длительность (кол-во дней)</label>
					<InputNumber
		...
		>
			...
			<template #footer>
				<Button label="Нет" icon="pi pi-times" text @click="deletePromoDialog = false" />
